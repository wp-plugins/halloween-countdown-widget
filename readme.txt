=== Halloween Countdown Widget ===

Contributors: monkeymays
Donate Link: http://christmaswebmaster.com/vampire-halloween-countdown-wordpress-plugin
Tags: halloween countdown, halloween, holiday, vampire, widget-only, widget
Requires at least: 3.0
Tested up to: 3.4.1
Stable tag: 1.1
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Displays a cute vampire Halloween countdown in your sidebar.



== Description ==

The Halloween Countdown Widget displays a cute vampire countdown to Halloween in your sidebar. 

 More information at [ChristmasWebmaster.com](http://christmaswebmaster.com/).

== Installation ==



1. Upload `hallocount.php` to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Go to your widgets screen, and drag the Halloween Countdown widget to your sidebar.
4. Let the countdown to Halloween begin!



== Frequently Asked Questions ==



= Does the bat have a name? =

Of course he does! We call him Vampy.

= Can I change the Countdown text color? =

Yes. You can edit the hallocount.php file CSS to suit your needs, find this line: color: #FFFF00; and edit accordingly.




== Screenshots ==


1. Upload plugin and install.
2. On the plugins panel activate the Halloween Countdown Widget.
3. In the widgets menu drag the Halloween Countdown Widget to your sidebar.
4. The Halloween Countdown Widget displays a cute Vampire countdown to Halloween in your sidebar!



== Changelog ==

= Version 1.1 =

*Released 10/24/2012

= Version 1.0 =

*Released 10/05/2012


== Upgrade Notice ==

= 1.1 =

*Updated countdown to fix date error.
*updated countdown to restart the countdown the day after halloween.
*updated countdown to display Happy Halloween!! on event daye.

= 1.0 =
*First Official Release